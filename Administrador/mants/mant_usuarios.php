...
if (isset($_POST['send_insert'])) {
    ...
    if ($_FILES['imagen']['type'] == "image/jpg" | $_FILES['imagen']['type'] == "image/jpeg" | $_FILES['imagen']['type'] == "image/png" | $_FILES['imagen']['type'] == "image/gif") {
        $target_path = "../../images/usuarios/";
        $nombre_final =  $nombre . $apellido . $tipo = $_FILES['imagen']['type'];
        $nombre_final2 = str_replace("image/", ".", $nombre_final);
        copy($_FILES['imagen']['tmp_name'], $target_path . $nombre_final2);
        $conn->insertar(
            "usuarios( nombre_usuario, apellido_usuario, correo_usuario,sexo,fecha_nacimiento,imagen_perfil,clave,tipo)",
            "'$nombre','$apellido','$correo','$sexo','$fecha','$nombre_final2','$clave','$tipo2'"
        );
        echo '<script>alert("Registro INGRESADO con exito");</script>';
        echo " <script>window.location.replace('../index.php?x=usuarios.php')</script>";
    } else {
        echo '<script>alert("Por favor registre una imagen valida.");</script>';
        echo " <script>window.location.replace('../index.php?x=usuarios.php')</script>";
    }
